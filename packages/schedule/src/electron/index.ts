    ...
    register(ctx) {
      ...
      try {
        ...
        ipcMain.handle(ScheduleChannels.LIST, async () =>
          withAuthenticatedValue(ctx, (requestContext) => controller.getAll(requestContext)),
        );
        ...
        ipcMain.handle(ScheduleChannels.LIST_BY_DATE_RANGE, async (_event, params) =>
          withAuthenticatedValue(ctx, (requestContext) =>
            controller.getByTimeRange(params ?? {}, requestContext),
          ),
        );
        ...
        ipcMain.handle(ScheduleChannels.UPDATE, async (_event, id, dto) =>
          withAuthenticatedValue(ctx, (requestContext) =>
            controller.update(id, dto, requestContext),
          ),
        );
        ...
        ipcMain.handle(ScheduleChannels.CREATE_WITH_CONFLICT_DETECTION, async (_event, request) =>
          withAuthenticatedValue(ctx, (requestContext) =>
            controller.createWithConflictDetection(request, requestContext),
          ),
        );
        ...
        ipcMain.handle(ScheduleChannels.RESOLVE_CONFLICT, async (_event, scheduleId, request) =>
          withAuthenticatedValue(ctx, (requestContext) =>
            controller.resolveConflict(scheduleId, request, requestContext),
          ),
        );
        ...
        void options.instance
          .dispose()
          .catch((disposeError) =>
            logger.error('Schedule dispose failed after IPC registration failure', disposeError),
          );
        throw error;
      }
    },
