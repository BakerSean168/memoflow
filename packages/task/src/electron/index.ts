 ...
 * `ipcMain` and owns that occurrence's start/dispose lifecycle.
 *
 * 本模块是传输适配器，而不是组合根：
 ...
 * occurrence in through `TaskElectronModuleOptions`. This factory never reads
 ...
 *
 * `occurrence.api` is the HTTP/IPC-shared application seam
 ...
 *
 * `occurrence.api` 是 HTTP/IPC 共用的应用 seam（`TaskApplicationPort`）。
 ...
 *   `occurrence.api`, registers all IPC handlers, then AWAITS `occurrence.start()`
 ...
 *   occurrence dispose (logged if dispose itself throws), moves to `failed`, and
 ...
 *   a terminal no-op too: the occurrence was already disposed and the installed
 ...
 *   BEFORE `await occurrence.dispose()` runs, so a reentrant/retry destroy stays
 *   a no-op even if dispose throws (destroy may propagate that error).
 *
 * 每个 handle 的状态机（`created -> registered | failed`，之后任意状态 ->
 * `disposed`）：
 * - register()：仅允许从 `created` 进入。用 `occurrence.api` 构建 controller、
 *   注册全部 IPC handler，然后 await `occurrence.start()`——handler 先于 start
 ...
 *   `await occurrence.dispose()`，因此即使 dispose 抛错（该错误可向外传播），
 *   重入/重试 destroy 仍为 no-op。
 *
 * The occurrence is owned by the factory closure, not by a package-level
 * singleton. Re-registering the returned module handle does not create a second
 * occurrence; the explicit state machine above is per-handle state.
 *
 ...
 * host-shaped: the factory returns it already bound to one occurrence.
 *
...
/**
 * Options carrying the already-assembled task occurrence.
 * 携带已装配任务实例的选项。
 */
export interface TaskElectronModuleOptions {
  readonly occurrence: TaskModuleInstance;
}
 ...
 * 见下方各 handler 注册。
 *
 * @param options - Options carrying the assembled task occurrence.
 * @returns An IElectronModule-compatible handle bound to the occurrence.
 */
export function createTaskElectronModule(
  options: TaskElectronModuleOptions,
): TaskElectronModuleDef {
  if (!options?.occurrence) {
    throw new Error('[FAIL-CLOSED] createTaskElectronModule requires options.occurrence');
  }
  ...
  return {
    ...
    async register(ctx) {
      ...
      try {
        const handlers = createTaskTransportHandlers(options.occurrence.api);
        ...
        registerValidatedChannel(
          ctx,
          TaskChannels.PLAN_LIST,
          ListTaskPlanFiltersSchema,
          (data, requestContext) => templateController.listPlans(data, requestContext),
          (params) =>
        ...
        installed.push(TaskChannels.PLAN_LIST);
        ipcMain.handle(TaskChannels.PLAN_GET, (_, payload) =>
          withAuthenticatedValue(ctx, async (requestContext) =>
            templateController.getPlan(payload?.id ?? payload, requestContext),
          ),
        );
        installed.push(TaskChannels.PLAN_GET);
        registerValidatedChannel(
          ctx,
          TaskChannels.PLAN_CREATE,
          CreateTaskPlanSchema,
          (data, requestContext) => templateController.createPlan(data, requestContext),
          (args) => args,
        );
        installed.push(TaskChannels.PLAN_CREATE);
        registerValidatedChannel(
          ctx,
          TaskChannels.PLAN_UPDATE,
          UpdateTaskPlanInvocationSchema,
          (data, requestContext) =>
            templateController.updatePlan(data.params.id, data.body, requestContext),
          (args) => ({
            params: { id: (args as { id?: string }).id },
            body: (args as { request?: unknown }).request,
          }),
        );
        installed.push(TaskChannels.PLAN_UPDATE);
        registerValidatedChannel(
          ctx,
          TaskChannels.PLAN_DELETE,
          TaskPlanIdCommandInvocationSchema,
          async (data, requestContext) => {
            const result = await templateController.deletePlan(data.params.id, requestContext);
            if (!result.ok) return result;
            return ok(null);
          },
          (args) => ({ params: { id: (args as { id?: string }).id ?? (args as string) } }),
        );
        installed.push(TaskChannels.PLAN_DELETE);
        registerValidatedChannel(
          ctx,
          TaskChannels.PLAN_ARCHIVE,
          TaskPlanIdCommandInvocationSchema,
          (data, requestContext) =>
            templateController.archivePlan(data.params.id, requestContext),
          (args) => ({ params: { id: (args as { id?: string }).id ?? (args as string) } }),
        );
        installed.push(TaskChannels.PLAN_ARCHIVE);
        registerValidatedChannel(
          ctx,
          TaskChannels.PLAN_ACTIVATE,
          TaskPlanIdCommandInvocationSchema,
          (data, requestContext) =>
            templateController.activatePlan(data.params.id, requestContext),
          (args) => ({ params: { id: (args as { id?: string }).id ?? (args as string) } }),
        );
        installed.push(TaskChannels.PLAN_ACTIVATE);
        registerValidatedChannel(
          ctx,
          TaskChannels.PLAN_ABANDON,
          AbandonTaskPlanInvocationSchema,
          (data, requestContext) =>
        ...
        installed.push(TaskChannels.PLAN_ABANDON);
        registerValidatedChannel(
          ctx,
          TaskChannels.PLAN_PAUSE,
          TaskPlanIdCommandInvocationSchema,
          (data, requestContext) =>
            templateController.pausePlan(data.params.id, requestContext),
          (args) => ({ params: { id: (args as { id?: string }).id ?? (args as string) } }),
        );
        installed.push(TaskChannels.PLAN_PAUSE);
        registerValidatedChannel(
          ctx,
          TaskChannels.PLAN_GENERATE_OCCURRENCES,
          GenerateOccurrencesInvocationSchema,
          (data, requestContext) =>
            templateController.generateOccurrences(data.params.id, data.body, requestContext),
          (args) => ({
            params: { id: (args as { planId?: string }).planId },
            body: (args as { request?: unknown }).request,
          }),
        );
        installed.push(TaskChannels.PLAN_GENERATE_OCCURRENCES);
        ipcMain.handle(TaskChannels.PLAN_GET_OCCURRENCES, (_, payload) =>
          withAuthenticatedValue(ctx, async (requestContext) => {
            return templateController.getOccurrencesByPlan(payload?.planId, requestContext, {
              from: payload?.from,
              to: payload?.to,
            });
          }),
        );
        installed.push(TaskChannels.PLAN_GET_OCCURRENCES);
        registerValidatedChannel(
          ctx,
          TaskChannels.PLAN_BIND_GOAL,
          BindTaskToGoalInvocationSchema,
          (data, requestContext) =>
        ...
        installed.push(TaskChannels.PLAN_BIND_GOAL);
        registerValidatedChannel(
          ctx,
          TaskChannels.PLAN_UNBIND_GOAL,
          TaskPlanIdCommandInvocationSchema,
          (data, requestContext) =>
            templateController.unbindFromGoal(data.params.id, requestContext),
          (args) => ({ params: { id: (args as { planId?: string }).planId } }),
        );
        installed.push(TaskChannels.PLAN_UNBIND_GOAL);

        // --- Instance channels ---
        ipcMain.handle(TaskChannels.OCCURRENCE_LIST, (_, params) =>
          withAuthenticatedValue(ctx, async (requestContext) => {
            if (params?.planId) {
              return handlers.occurrence.listByTemplate(params.planId, requestContext.identityId);
            }

            if (params?.status) {
              return handlers.occurrence.listByStatus(requestContext.identityId, params.status);
            }

            return handlers.occurrence.listByAccount(requestContext.identityId);
          }),
        );
        ...
        ipcMain.handle(TaskChannels.OCCURRENCE_GET, (_, payload) =>
          withAuthenticatedValue(ctx, async (requestContext) =>
            instanceController.getOccurrence(payload?.id ?? payload, requestContext),
          ),
        );
        installed.push(TaskChannels.OCCURRENCE_GET);
        registerValidatedChannel(
          ctx,
          TaskChannels.OCCURRENCE_CREATE,
          TaskOccurrenceIdCommandInvocationSchema,
          (data, requestContext) =>
            instanceController.startOccurrence(data.params.id, requestContext),
          (args) => ({ params: { id: (args as { id?: string }).id ?? (args as string) } }),
        );
        installed.push(TaskChannels.OCCURRENCE_CREATE);
        registerValidatedChannel(
          ctx,
          TaskChannels.OCCURRENCE_DELETE,
          TaskOccurrenceIdCommandInvocationSchema,
          async (data, requestContext) => {
            const result = await instanceController.deleteOccurrence(data.params.id, requestContext);
            if (!result.ok) return result;
            return ok(null);
          },
          (args) => ({ params: { id: (args as { id?: string }).id ?? (args as string) } }),
        );
        installed.push(TaskChannels.OCCURRENCE_DELETE);
        registerValidatedChannel(
          ctx,
          TaskChannels.OCCURRENCE_COMPLETE,
          CompleteTaskOccurrenceInvocationSchema,
          (data, requestContext) =>
            instanceController.completeOccurrence(data.params.id, data.body, requestContext),
          (args) => ({
            params: { id: (args as { id?: string }).id ?? (args as string) },
            body: (args as { request?: unknown }).request,
          }),
        );
        installed.push(TaskChannels.OCCURRENCE_COMPLETE);
        registerValidatedChannel(
          ctx,
          TaskChannels.OCCURRENCE_UNCOMPLETE,
          TaskOccurrenceIdCommandInvocationSchema,
          (data, requestContext) =>
            instanceController.uncompleteOccurrence(data.params.id, requestContext),
          (args) => ({ params: { id: (args as { id?: string }).id ?? (args as string) } }),
        );
        installed.push(TaskChannels.OCCURRENCE_UNCOMPLETE);
        registerValidatedChannel(
          ctx,
          TaskChannels.OCCURRENCE_SKIP,
          SkipTaskOccurrenceInvocationSchema,
          (data, requestContext) =>
            instanceController.skipOccurrence(data.params.id, data.body, requestContext),
          (args) => ({
            params: { id: (args as { id?: string }).id ?? (args as string) },
            body: (args as { request?: unknown }).request,
          }),
        );
        installed.push(TaskChannels.OCCURRENCE_SKIP);
        registerValidatedChannel(
          ctx,
          TaskChannels.OCCURRENCE_MARK_MISSED,
          MarkTaskOccurrenceMissedInvocationSchema,
          (data, requestContext) =>
            instanceController.markOccurrenceMissed(data.params.id, data.body, requestContext),
          (args) => ({
            params: { id: (args as { id?: string }).id ?? (args as string) },
            body: (args as { request?: unknown }).request,
          }),
        );
        installed.push(TaskChannels.OCCURRENCE_MARK_MISSED);
        registerValidatedChannel(
          ctx,
          TaskChannels.OCCURRENCE_CHECKLIST_SET,
          SetTaskOccurrenceChecklistItemInvocationSchema,
          (data, requestContext) =>
            instanceController.setOccurrenceChecklistItem(data.params.id, data.body, requestContext),
          (args) => ({
            params: { id: (args as { id?: string }).id ?? (args as string) },
            body: (args as { request?: unknown }).request,
          }),
        );
        installed.push(TaskChannels.OCCURRENCE_CHECKLIST_SET);
        registerValidatedChannel(
          ctx,
          TaskChannels.OCCURRENCE_RESCHEDULE,
          RescheduleTaskOccurrenceInvocationSchema,
          (data, requestContext) =>
            instanceController.rescheduleOccurrence(data.params.id, data.body, requestContext),
          (args) => {
        ...
        installed.push(TaskChannels.OCCURRENCE_RESCHEDULE);

        await options.occurrence.start();
        state = 'registered';
        ...
        try {
          await options.occurrence.dispose();
        } catch (disposeError) {
          logger.error(
            'TaskElectron: occurrence dispose failed during failed registration',
            disposeError,
          );
        }
        throw error;
