 ...
 * `ipcMain` and owns that module instance's start/dispose lifecycle.
 *
 * 本模块是传输适配器，而不是组合根：
 ...
 * module instance in through `TaskElectronModuleOptions`. This factory never reads
 ...
 *
 * `moduleInstance.api` is the HTTP/IPC-shared application seam
 ...
 *
 * `moduleInstance.api` 是 HTTP/IPC 共用的应用 seam（`TaskApplicationPort`）。
 ...
 *   `moduleInstance.api`, registers all IPC handlers, then AWAITS `moduleInstance.start()`
 ...
 *   module instance dispose (logged if dispose itself throws), moves to `failed`, and
 ...
 *   BEFORE `await moduleInstance.dispose()` runs, so a reentrant/retry destroy stays
 ...
 *   注册全部 IPC handler，然后 await `moduleInstance.start()`——handler 先于 start
 ...
 *   `await moduleInstance.dispose()`，因此即使 dispose 抛错（该错误可向外传播），
 *   重入/重试 destroy 仍为 no-op。
 *
 * The module instance is owned by the factory closure, not by a package-level
 * singleton. Re-registering the returned module handle does not create a second
 * module instance; the explicit state machine above is per-handle state.
 *
 ...
 * 见下方各 handler 注册。
 *
 * @param options - Options carrying the assembled task module instance.
 * @returns An IElectronModule-compatible handle bound to the module instance.
 */
    ...
    async register(ctx) {
      ...
      try {
        ...
        try {
          ...
          logger.error(
            'TaskElectron: module instance dispose failed during failed registration',
            disposeError,
          );
        }
        throw error;
