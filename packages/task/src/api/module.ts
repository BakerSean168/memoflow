 ...
 * router and owns that occurrence's start/dispose lifecycle.
 *
 * 本模块是传输适配器，而不是组合根：
 ...
 * occurrence in through `TaskApiModuleOptions`. This factory never reads
 ...
 *   `occurrence.api` and the fixed routes (`/task-plans`,
 *   `/task-occurrences`) via `registerTaskRoutes`, AWAITS
 *   `occurrence.start()`, and ONLY THEN mounts the combined router — a failed
 ...
 *   a terminal no-op too: the occurrence was already disposed in the register()
 *   failure path. For a live handle the state is set to `disposed` BEFORE
 *   `await occurrence.dispose()` runs, so a reentrant/retry destroy stays a no-op
 *   even if dispose throws (destroy may propagate that error).
 *
 * 每个 handle 的状态机（`created -> registered | failed`，之后任意状态 ->
 * `disposed`）：
 * - register()：仅允许从 `created` 进入。用 `occurrence.api` 构建控制器并通过
 *   `registerTaskRoutes` 挂载固定路由（`/task-plans`、`/task-occurrences`、
 *   `/tasks`），await `occurrence.start()`，之后才挂载组合 router——start 失败
 ...
 *   在 `await occurrence.dispose()` 执行前先把状态置为 `disposed`，因此即使
 ...
 * @returns An IApiModule-compatible handle bound to the occurrence.
 */
export function createTaskApiModule(options: TaskApiModuleOptions): TaskApiModuleDef {
  if (!options?.occurrence) {
    throw new Error('[FAIL-CLOSED] createTaskApiModule requires options.occurrence');
  }
    ...
    async register(context) {
      ...
      try {
        ...
        try {
          ...
          logger.error(
            'TaskApiModule: occurrence dispose failed during failed registration',
            disposeError,
          );
        }
        throw error;
