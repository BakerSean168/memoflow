 ...
 * Sole body in get-first-query-value.ts; occurrence + plan routes import it.
...
describe('getFirstQueryValue dual retired (residual 983)', () => {
  ...
  const occurrence = readFileSync(resolve(dir, 'task-occurrence.routes.ts'), 'utf8');
  const plan = readFileSync(resolve(dir, 'task-plan.routes.ts'), 'utf8');
  ...
  it('occurrence + plan routes import sole without local dual bodies', () => {
