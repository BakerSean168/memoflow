...
export async function importGoals(
  ...
  for (const goal of data.items) {
    ...
    await tx.createGoal({
      ...
      targetEndDate: target ? goalTimeframeEndBoundary(target) : null,
    ...
    for (const review of (g.goalReviews as unknown[]) ?? []) {
      ...
      const systemContext: GoalReviewSystemContext = {
        ...
        keyResults: portableContext.keyResults.map((item) => ({
          ...item,
          keyResultId: resolveRef(
            item.keyResultRef,
            ctx,
          ) as GoalReviewSystemContext['keyResults'][number]['keyResultId'],
        })),
      };
