...
function fixture() {
  ...
  return {
    adapter: new ControlledAnalyticsReadAdapter({
      goalApplicationPort,
...
describe('ControlledAnalyticsReadAdapter', () => {
  it('composes bounded owner projections without a Dashboard context', async () => {
    vi.useFakeTimers();
    ...
    try {
      ...
      expect(context.ownerReads).toEqual({
        goal: { progress: { activeCount: 2, goals: [] } },
      ...
      expect(context.taskDashboard?.summary.overdue).toBe(1);
