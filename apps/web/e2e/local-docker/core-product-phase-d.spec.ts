...
test.describe('Local Docker core product Phase D', () => {
  test('[P2] completes the shared Goal/KR/Task forms with keyboard and accessible product language', async ({
    ...
    await expect(page.getByTestId('goal-start-chip-query')).toBeVisible();
    ...
    await expect(page.getByTestId('goal-target-chip-query')).toBeVisible();
    await expect(page.getByRole('textbox', { name: '目标时间', exact: true })).toBeVisible();
    await page.keyboard.press('Escape');
    ...
    const krTargetQuery = page.getByTestId('draft-kr-target-timeframe-query');
    await expect(krTargetQuery).toBeVisible();
    await krTargetQuery.fill('2027-12-31');
    await krTargetQuery.press('Enter');
    await page.keyboard.press('Escape');
