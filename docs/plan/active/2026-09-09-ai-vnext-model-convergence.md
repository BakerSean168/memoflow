...
| Data Portability export                         | The owner capability exports only the current AI conversation shell (`ref`, `name`, `status`) as `ai-conversations@3`; transcript/runtime state is not portable | Current owner-driven V3 fact, validated and ordered by the shared coordinator. The former persistence-shaped exporter/projection was deleted by PORT-1611. (`packages/ai/src/server/application/ai-conversation-portability.ts`) |
| Data Portability import                         | `ai-conversations@3` validates shell payloads, allocates destination-owned conversation ids, and never restores transcript/runtime state | Current owner-driven V3 apply path; no legacy `AiMessage` importer, V2 DTO, mini repository, migration, or backfill remains under ADR-111. (`packages/ai/src/server/application/ai-conversation-portability.ts`) |
| Domain and contract message model               | `AIConversation.messages`, `Message` entities, `AiMessageId`, server/client DTOs and AI response schemas                                         | Derived/recoverable active legacy model surface used by shell repositories, but not by Mastra runtime authority. PORT-1611 removed its Data Portability consumers; remaining domain/schema cleanup is owned by the later coordinated schema cutover. (`packages/ai/src/server/domain/aggregates/ai-conversation.ts`, `packages/ai/src/server/domain/entities/message.ts`, `packages/ai/src/domain-client/entities/message.ts`, `packages/contracts/src/modules/ai/entities/message-server.ts`, `packages/contracts/src/modules/ai/api/response-schemas.ts`) |
