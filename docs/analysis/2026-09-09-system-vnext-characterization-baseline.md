...
| Dashboard            | `/dashboard` is compatibility redirect only; Home/Goal and AI analytics use owner reads; HOME-1804 deletes the durable cross-domain activity ledger and makes API recent activity owner-derived like Desktop, leaving Dashboard itself for HOME-1805 | `dashboard-retirement-characterization.surface.spec.ts`, `owner-activity-read.adapter.spec.ts`, `TodayOverviewPanel.spec.ts`        |
