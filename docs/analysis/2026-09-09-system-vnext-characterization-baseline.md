...
| Dashboard            | `/dashboard` is compatibility redirect only; API/Desktop dashboard endpoints remain, while Home/Goal and AI analytics now use owner reads; the remaining AI-side legacy fact is the narrow API ActivityLedger capability pending HOME-1804 | `dashboard-retirement-characterization.surface.spec.ts`, `TodayOverviewPanel.spec.ts`, `GoalCapsulePreview.spec.ts`                 |
