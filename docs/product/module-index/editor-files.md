...
`EDITOR-1701/1702` 已完成 destructive cutover：Editor 不再属于 V2/V3 portability，
`editor_*` Prisma/PowerSync 表、Account 关系、同步规则、natural-key bootstrap、portable DTO、
projection/importer/adapters/import-store seam 均已删除。旧备份中的 `data.editor` 会严格校验失败，
不提供兼容 warning、数据恢复或隐藏写回路径。

| 文件                                                                                                                                                                                                                                                | 说明                                                              |
| --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------- |
| [`packages/data-portability/src/server/application/use-cases/__tests__/editor-portability-retirement.surface.spec.ts`](../../../packages/data-portability/src/server/application/use-cases/__tests__/editor-portability-retirement.surface.spec.ts) | Editor 不进入 V2/V3 portability 的退役锁                          |
| [`packages/data-portability/src/server/application/use-cases/__tests__/editor-persistence-retirement.surface.spec.ts`](../../../packages/data-portability/src/server/application/use-cases/__tests__/editor-persistence-retirement.surface.spec.ts) | Prisma / PowerSync / portability persistence anti-resurrection 锁 |
| [`packages/powersync-schema/src/index.spec.ts`](../../../packages/powersync-schema/src/index.spec.ts)                                                                                                                                               | 显式断言四张 `editor_*` 表不存在                                  |
| [`apps/migrator/src/main.test.ts`](../../../apps/migrator/src/main.test.ts)                                                                                                                                                                         | 迁移启动序列不再执行 Editor natural-key bootstrap                 |
...
- 不要重新引入 `editor_*` 表、Editor portable payload、projection/importer 或 natural-key bootstrap。
- 不要恢复 `/note/:id` 或挂载旧 Editor API / Electron runtime。
- Web Markdown 必须继续走 sanitizer；禁止重新启用原始 HTML。
- 旧备份中的 Editor payload 必须继续 fail closed；不要添加兼容恢复路径。
