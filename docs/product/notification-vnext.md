...
> **Current truth (2026-09-19):** `NotificationChannelType` is only a channel-kind value used by delivery policy; it is not a NotificationChannel aggregate or persistence owner. The retired aggregate/table, template, history and generic action surfaces are absent from current runtime/schema.
...
以下是 2026-09-08 cutover 前的第一代模型残差清单，保留用于解释为什么本轮采用当前边界：
...
## 5. NotificationFact (current shape)
